/*!
 * \file src/hid/gtk3/gtk3d-main.c
 *
 * \brief GTK3 GUI.
 *
...
/*!
 * \brief .
 * \warning Don't pan so far the board is completely off the screen.
 */
static void
 ...
 * zoom value is PCB units per screen pixel.
 * Larger numbers mean zooming out - the largest value means you are
...
/*!
 * \brief Arbitrary, and same as the GTK2 and lesstif HID.
 */
#define ALLOW_ZOOM_OUT_BY 10 


/*!
 * \brief Set absolute zoom view.
 */
static void
...
/*!
 * \brief Set zoom syntax text string.
 */
...
/*!
 * \brief Set zoom help text string.
 */
...
%end-doc */
/*!
 * \brief Set zoom factor for the view of the layout.
 */
static int
 ...
 * \brief Calibrate.
 *
 * There is no use case for calibrating a GUI.
...
/*!
 * \brief Set the \c is_up flag for the gui.
 */
void
...
ghid_set_crosshair (int x, int y, int action)
{
  ...
  if (gport->crosshair_x != x || gport->crosshair_y != y)
  {
    ...
    /*!
     * \todo does this trigger the idle_proc stuff?
     * It is in the lesstif HID.
     * Maybe something is needed here?
     * need_idle_proc ();
     */
  }
  ...
  /* Find out where the drawing area is on the screen.
   * gdk_display_get_pointer  and gdk_display_warp_pointer work relative
   * to the whole display, whilst our coordinates are relative to the
   * drawing area origin.
   */
  ...
  switch (action)
  {
    ...
    case HID_SC_CENTER_IN_VIEWPORT_AND_WARP_POINTER:
       ...
       * widget coordinates so gdk_display_warp_pointer will go to the
       * right spot.
       */
...
/*!
 * \brief We need a wrapper around the hid timer because a gtk timer
 * needs to return FALSE else the timer will be restarted.
 */
static gboolean
...
/*!
 * \brief We need a wrapper around the hid file watch to pass the
 * correct flags
 */
static gboolean
...
(
  ...
  void (*func) (hidval watch, int fd, unsigned int condition, hidval user_data),
  hidval user_data
)
{
...
/*!
 * \brief Prepare a block hook.
 */
static gboolean
ghid_block_hook_prepare (GSource *source, gint *timeout)
{
...
/*!
 * \brief Dispatch a block hook.
 */
static gboolean
ghid_block_hook_dispatch (GSource *source, GSourceFunc callback, gpointer user_data)
{
  return FALSE;
}
...
/*!
 * \brief Add a block hook.
 */
static hidval
ghid_add_block_hook (void (*func) (hidval data), hidval user_data)
{
  hidval ret;
...
/*!
 * \todo implement a proper file select dialog.
 */
#ifdef FIXME
char *
ghid_fileselect
...
/*!
 * \brief Run the Response handler for the progress dialog.
 */
static void
run_response_handler (GtkDialog *dialog, gint response_id, gpointer data)
{
...
/*!
 * \brief Run the Delete handler for the progress dialog.
 */
static gint
...
/*!
 * \brief Run the Destroy handler for the progress dialog.
 */
static void
 ...
 *
 * Make the dialog modal so nothing else can get events whilst the main
 * loop isn't running.
 */
...
make_progress_dialog (void)
{
  ...
  /*! \todo replace with gtk_widget_get_preferred_height or gtk_widget_get_preferred_width. */
  ...
  /*! \todo replace with gtk_widget_get_preferred_height or
  ...
  pd->response_handler = g_signal_connect (pd->dialog, "response",
                                           G_CALLBACK (run_response_handler), pd);

  pd->delete_handler = g_signal_connect (pd->dialog, "delete-event",
                                         G_CALLBACK (run_delete_handler), pd);

  pd->destroy_handler =  g_signal_connect (pd->dialog, "destroy",
...
/*!
 * \brief Destroy a progress dialog.
 */
static void
...
/*!
 * \brief Handle progress dialog events.
 */
static void
...
#define MIN_TIME_SEPARATION (50./1000.) /*!< 50 milliseconds. */


/*!
 * \brief Handle progress dialog.
 */
static int
...
/*!
 * \brief Set the table size for attributes table.
 */
static void
...
/*!
 * \brief Add attribute rows in the attribute table if maximum rows value is increased.
 */
static void
ghid_attributes_need_rows (int new_max)
{
  ...
  while (attr_max_rows < new_max)
  {
    ...
    gtk_table_attach (GTK_TABLE (attr_table),
                      attr_row[attr_max_rows].del,
                      0, 1,
    ...
    g_signal_connect
    (
      G_OBJECT (attr_row[attr_max_rows].del),
      "clicked",
      G_CALLBACK (attributes_delete_callback),
    ...
    gtk_table_attach (GTK_TABLE (attr_table),
                      attr_row[attr_max_rows].w_value,
                      2, 3,
                      attr_max_rows, attr_max_rows + 1,
                      (GtkAttachOptions)(GTK_FILL | GTK_EXPAND), GTK_FILL,
                      0, 0);
...
/*!
 * \brief Clean up unused attribute rows.
 */
static void
...
/*!
 * \brief Attributes delete callback.
 */
static void
...
/*!
 * \brief Attributes dialog handler.
 */
static void
...
RouteStylesChanged (int argc, char **argv, Coord x, Coord y)
{
  ...
  ghid_route_style_selector_sync (GHID_ROUTE_STYLE_SELECTOR (ghidgui->route_style_selector),
...
/*!
 * \brief No need to show the library window every time it changes...
 *
...
static const char load_help[] = N_("Load layout data from a user-selected file.");
...
Save (int argc, char **argv, Coord x, Coord y)
{
  ...
  name = ghid_dialog_file_select_save (prompt, &current_dir,
  ...
  if (name)
  {
    ...
    {
      /* If we got this far and the function is Layout, then we really
       * needed it to be a LayoutAs.
       * Otherwise  ActionSaveTo() will ignore the new file name we just
       * obtained.
       */
      if (strcasecmp (function, "Layout") == 0)
...
Benchmark (int argc, char **argv, Coord x, Coord y)
{
  ...
  display = gdk_window_get_display (gport->drawable);
...
#undef REST_OF_CACHE
  ...
  ghid_hid.name                     = "gtk3";
  ghid_hid.description              = "GTK3 - The Gimp Toolkit";
  ghid_hid.gui                      = 1;
