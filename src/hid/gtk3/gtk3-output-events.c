/*!
 * \file src/hid/gtk3/gtk3-output-events.c
 *
 * \brief Provides handling of the output events for the GTK3 UI.
 *
...
/*!
 * \brief Do scrollbar scaling based on current port drawing area size
 * and overall PCB board size.
 */
void
ghid_port_ranges_scale (void)
{
  ...
  /* Update the scrollbars with PCB units.
   * So Scale the current drawing area size in pixels to PCB units and
   * that will be the page size for the Gtk adjustment. */
...
gboolean
ghid_port_key_release_cb (GtkWidget * drawing_area, GdkEventKey * kev, gpointer data)
{
...
/*!
 * \brief Handle user keys in the output drawing area.
 *
 ...
 */
gboolean
ghid_port_key_press_cb (GtkWidget *drawing_area, GdkEventKey *kev, gpointer data)
{
...
gboolean
ghid_port_button_press_cb (GtkWidget *drawing_area, GdkEventButton *ev, gpointer data)
{
  ModifierKeysState mk;
  GdkModifierType state;
...
gboolean
ghid_port_button_release_cb (GtkWidget *drawing_area, GdkEventButton *ev, gpointer data)
{
  ModifierKeysState mk;
  GdkModifierType state;
...
gboolean
ghid_port_drawing_area_configure_event_cb (GtkWidget *widget, GdkEventConfigure *ev, GHidPort *out)
{
...
/*!
 * \todo If the GHidPort is ever destroyed, we must call
...
queue_tooltip_update (GHidPort *out)
{
  ...
  out->tooltip_update_timeout_id = g_timeout_add (TOOLTIP_UPDATE_DELAY,
...
gint
ghid_port_window_motion_cb (GtkWidget *widget, GdkEventMotion *ev, GHidPort *out)
{
  gdouble dx;
  gdouble dy;
...
gint
ghid_port_window_enter_cb (GtkWidget *widget, GdkEventCrossing *ev, GHidPort *out)
{
...
gint
ghid_port_window_leave_cb (GtkWidget *widget, GdkEventCrossing *ev, GHidPort *out)
{
...
/*!
 * \brief Mouse scroll wheel events.
 */
gint
ghid_port_window_mouse_scroll_cb (GtkWidget *widget, GdkEventScroll *ev, GHidPort *out)
{
  ModifierKeysState mk;
  GdkModifierType state;
  int button;
